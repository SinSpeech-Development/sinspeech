...
# skip the same parameter setup as described before
gmm_dir=exp/${gmm}  # final hmm model directory

# training alignment directory (does not exist yet) for speed-perturbed data
...
if [ $stage -le 3 ]; then
  ...
  for datadir in ${train_set}_sp test valid; do
